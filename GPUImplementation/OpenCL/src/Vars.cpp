...
{
	d->sol = (double*)malloc(sizeof(double) * g->size_x * g->size_y);
	d->radius = r;
	d->alpha = a;
}
...
void initCells(Grid* g, Domain* d, Cells* c)
{
	...
	c->x = (double*)malloc(sizeof(double) * nn);
	c->y = (double*)malloc(sizeof(double) * nn);
	c->xc = (double*)malloc(sizeof(double) * nn);
	c->yc = (double*)malloc(sizeof(double) * nn);
	c->sol_rows = (double*)malloc(sizeof(double) * nn);
	c->sol_cols = (double*)malloc(sizeof(double) * nn);
	c->code = (int*)malloc(sizeof(int) * nn);
	c->fArray = (double*)malloc(sizeof(double) * nn);
	c->fArrayc = (double*)malloc(sizeof(double) * nn);
	...
	double inv_x = g->max_x / (double)nx;
	double inv_y = g->max_y / (double)ny;
	double inv_x2 = inv_x * 0.5;
	double inv_y2 = inv_y * 0.5;
	for (int j = 0; j < ny; j++)
	{
		for (int i = 0; i < nx; i++)
		{
			double x = i * inv_x + (inv_x2), y = j * inv_y + (inv_y2);
			c->x[i + j * nx] = x;
			c->y[i + j * nx] = y;
	...
	for (int j = 0; j < ny; j++)
	{
		for (int i = 0; i < nx; i++)
		{
			id = i + j * nx;
			id2 = i * ny + j;
			double val = get_ExactValue(c->x[id], c->y[id]);
			if (c->code[id] == 0)
			{
				c->sol_rows[id] = 0.0;
				c->sol_cols[id2] = 0.0;
