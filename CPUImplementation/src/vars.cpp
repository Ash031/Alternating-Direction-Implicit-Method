...
//initialize the auxiliar structure for solving ADI
void initAux(Domain *d, Grid *g, AuxSolver *a)
{
	//int x = d->max_x - d->min_x + 1, y = d->max_y - d->min_y + 1, xy = x*y;
	int nx = g->size_x, ny = g->size_y, nn = (nx>ny ? nx : ny);	
	a->known = (real*)_mm_malloc(sizeof(real)*nn,sizeof(real));
	a->a = (real*)_mm_malloc(sizeof(real)*nn,sizeof(real));
	a->b = (real*)_mm_malloc(sizeof(real)*nn,sizeof(real));
	a->c = (real*)_mm_malloc(sizeof(real)*nn,sizeof(real));
}
...
void initGridAndDomain(Grid* g, Domain* d){
	...
	initGrid(g, 0.0, 0.0, 2.0, 2.0, 2560, 2560);
